...
class WebServer {
    constructor(options) {
        ...
        this.converter = {
            ...
            [Types.hue]:                    this._processLightAdvanced.bind(this),
            [Types.rgb]:                    this._processLightAdvanced.bind(this),
            [Types.rgbSingle]:              this._processLightAdvanced.bind(this),
    ...
    _getAllEntities(cb) {
        this.adapter.getObjectView('custom', 'state', {}, (err, doc) => {
            const ids = [];
            if (doc && doc.rows) {
    ...
    _setLightAdvancedAttributesToIOBStates(data, entity, user, resolve, reject) {
        ...
        if (data.service_data.color_temp) { //will also be false for ct = 0 -> but ct = 0 is not a useful mired value and creates issues with the conversion.
            ...
            this.adapter.setForeignState(attr.setId, ct, false, {user}, err =>
                err ? reject(err) : resolve());
            return;
        }
        if (data.service_data.brightness >= 0) {
            ...
            this.adapter.setForeignState(attr.setId, data.service_data.brightness / 255 * attr.max, false, {user}, err =>
                err ? reject(err) : resolve());
            return;
        }
        if (data.service_data.brightness_pct >= 0) {
            ...
            this.adapter.setForeignState(attr.setId, data.service_data.brightness_pct / 100 * attr.max, false, {user}, err =>
                err ? reject(err) : resolve());
            return;
        }
        if (data.service_data.hs_color) {
            ...
            if (attr) {
                if (attr.is_rgb_string || attr.is_rgb_values) {
                    ...
                    const rgb = this._HSV2RGB(h, s, v);
                    if (attr.is_rgb_string) {
                        const rgbString = '#' + NumToHex(rgb.r) + NumToHex(rgb.g) + NumToHex(rgb.b);
                        this.adapter.setForeignState(attr.setId, rgbString, false, {user}, err => err ? reject(err) : resolve());
                        ...
                        this.adapter.setForeignState(attr.setId, rgb.r / 255 * attr.max, false, {user}, err => err ? reject(err) :
                            this.adapter.setForeignState(attrGreen.setId, rgb.g / 255 * attr.max, false, {user}, err => err ? reject(err) :
                                this.adapter.setForeignState(attrBlue.setId, rgb.b / 255 * attr.max, false, {user}, err => err ? reject(err) : resolve())));
                    }
                } else {
                    this.adapter.setForeignState(attr.setId, h / 360 * attr.max, false, {user}, err => {
                        if (err) {
                            reject(err);
                        } else {
                            if (attr_Sat) {
                                this.adapter.setForeignState(attr_Sat.setId, s / 100 * attr_Sat.max, false, {user}, err => err ? reject(err) : resolve());
                            } else {
    ...
    _parseLightAdvancedOn(entity, command, data, user) {
                ...
                this.adapter.getForeignStates(ids, {user}, (err, state) => {
                    ...
                    if (!state[entity.context.STATE.getId] || !state[entity.context.STATE.getId].val) {
                        ...
                        this.adapter.setForeignState(entity.context.STATE.setId, true, false, {user}, () =>
                            this._setLightAdvancedAttributesToIOBStates(data, entity, user,resolve, reject));
                    } else {
                        this._setLightAdvancedAttributesToIOBStates(data, entity, user,resolve, reject);
                    }
                });
    ...
    _lightAdvancedAddBrightness(control, objects, entity) {
        ...
        if (!state) {
            ...
            if (state.type !== 'number') { //in dimmer SET is for level for everything else it is on/off.
                state = undefined;
            }
        }
        if (state && state.id) {
    ...
    _lightAdvancedAddColorTemperature(control, objects, entity) {
    ...
    _getLightAdvancedState(control) {
        function getState(name) {
            const state = control.states.find(s => s.id && s.name === name);
            if (state && state.id && state.type === 'boolean') {
                return state;
            }
            return undefined;
        }

        let state = getState('ON');
        ...
        if (!state) {
            state = getState('ON_LIGHT');
        }
        return state;
    }

    _lightAdvancedAddState(control, state, entity) {
    ...
    _lightAdvancedAddHueAndSat(control, objects, entity) {
    ...
    _lightAdvancedAddRGBSingle(control, objects, entity) {
    ...
    _lightAdvancedAddRGB(control, objects, entity) {
        if (entity.context.ATTRIBUTES && entity.context.ATTRIBUTES.find((a) => a.attribute === 'hs_color')) {
    ...
    _processLightAdvanced(id, control, name, room, func, _obj, objects) {
        ...
        if (state && state.id) {
            ...
            this._lightAdvancedAddColorTemperature(control, objects, entity);

            //if there is a "BRIGHTNESS" control, use it to dim lamp.
            this._lightAdvancedAddBrightness(control, objects, entity);

            //add hue and sat:
            this._lightAdvancedAddHueAndSat(control, objects, entity);

            //add rgb. Will only happen, if no hue.
            this._lightAdvancedAddRGBSingle(control, objects, entity);

            //add rgb as single states. Will only happen if no hue and no rgbSingle:
            this._lightAdvancedAddRGB(control, objects, entity);
    ...
    _processManualLight(id, obj, entity) {
        let promise;
        ...
        if (obj.common.type === 'number') {
            ...
            promise = this.adapter.getForeignStateAsync(id).then((state) => {
                entity.attributes.brightness = state.val;
                entity.context.ATTRIBUTES = [{attribute: 'brightness', getId: id, setId: id}];
                entity.context.COMMANDS = [{
                    service: 'turn_on',
                    setId: id,
                    on: iobMaxValue,
                    parseCommand: (entity, command, data, user) => {
                        ...
                        if (data.service_data.brightness >= 0) {
                            entity.attributes.brightness = data.service_data.brightness;
                            entity.attributes.brightness_pct = data.service_data.brightness / 255;
                            val = data.service_data.brightness / 255 * command.on;
                        }
                        if (data.service_data.brightness_pct >= 0) {
                            entity.attributes.brightness = (data.service_data.brightness_pct / 100) * 255;
                            entity.attributes.brightness_pct = data.service_data.brightness_pct;
                            val = data.service_data.brightness_pct / 100 * command.on;
                        }
    ...
    _processManualEntity(id, _entities, cb) {
        let promise = this.adapter.getForeignObjectAsync(id);
        ...
        promise = promise.then((obj) => {
            ...
            entity = this._processCommon(id, null, null, null, obj, entityType, entityType + '.' + name);

            entity.context.STATE = {getId: id, setId: id};
            entity.isManual = true;

            if (entityType === 'light') {
                return this._processManualLight(id, obj, entity);
                ...
                return this.adapter.getForeignStateAsync(id).then((state) => entity.attributes.initial = state.val);
            } else if (entityType === 'input_boolean') {
                return this.adapter.getForeignStateAsync(id).then((state) =>
        ...
        promise.then(() => {
            ...
            this.log.error(`Could not manually create entity ${id}: ${err} - ${err.stack}`);
            cb && cb();
        });
    }
    ...
    _readObjects() {
        return new Promise(resolve => {
            ...
            this.adapter.getObjectView('system', 'state', params, (err, _states) => {
                this.adapter.getObjectView('system', 'channel', params, (err, _channels) => {
                    this.adapter.getObjectView('system', 'device', params, (err, _devices) => {
